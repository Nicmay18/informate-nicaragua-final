...
async function main() {
  console.log('📡 Buscando noticias en Firestore...');
  ...
  for (const reescrita of noticiasReescritas) {
    const encontrada = todas.find(n => n.titulo === reescrita.titulo);
    if (encontrada) {
      console.log(`✅ Encontrada: ${reescrita.titulo} (ID: ${encontrada.id})`);
      await updateDoc(doc(db, 'noticias', encontrada.id), {
        titulo: reescrita.titulo,
        resumen: reescrita.resumen,
        contenido: reescrita.cuerpo,
        nivel: reescrita.nivel,
        score: reescrita.score,
        fechaActualizacion: new Date()
      });
      console.log(`   → Guardada como ORO`);
      guardadas++;
    } else {
      console.log(`❌ NO encontrada: ${reescrita.titulo}`);
      noEncontradas.push(reescrita.titulo);
    }
  }
