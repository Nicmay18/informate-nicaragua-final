...
async function main() {
  ...
  for (let i = 0; i < docs.length; i++) {
    ...
    const categoria = (data.categoria || 'Actualidad').trim();
    ...
    if ((i + 1) % 50 === 0) {
      console.log(`Auditadas ${i + 1}/${docs.length}...`);
    }
  }
  ...
  console.log(`\nTotal: ${resultados.length}`);
  console.log(`Aprobadas: ${resultados.filter(r => r.aprobada).length}`);
  console.log(`Reprobadas: ${resultados.filter(r => !r.aprobada).length}`);
  console.log(`Resultado guardado en auditoria-228-resultado.json`);
}
