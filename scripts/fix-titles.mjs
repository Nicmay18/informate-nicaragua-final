...
const keyPath = 'G:\\RESPALDO\\informate-instant-nicaragua-firebase-adminsdk-fbsvc-2da99059f4.json';
...
async function main() {
  ...
  for (const [oldTitle, newTitle] of TITLES) {
    try {
      ...
      console.log(`✅ UPDATED: "${oldTitle.substring(0, 50)}..." → "${newTitle.substring(0, 50)}..."`);
      updated++;
    } catch (err) {
      console.error(`❌ ERROR: "${oldTitle}"`, err.message);
    }
  }
