          ...
          <div className="space-y-6">
            ...
            <Card title="CEO Verdict" icon={ceoVerdict.statusIcon}>
              <div className="space-y-4">
                <div className={`text-3xl font-bold ${STATUS_TEXT[ceoVerdict.status === 'SALUDABLE' ? 'ok' : ceoVerdict.status === 'REQUIERE_ATENCION' ? 'warning' : ceoVerdict.status === 'RIESGO_CRITICO' ? 'critical' : 'missing']}`}>
            ...
            {/* Growth Engine — Plan de Hoy (N5) */}
            <Card title="Growth Engine — Plan de Hoy" icon="🚀">
              {growth && growth.plan.length > 0 ? (
                <div className="space-y-4">
                  <p className="text-sm text-slate-700">{growth.speaks}</p>
                  <ol className="space-y-3">
                    {growth.plan.map((item) => (
                      <li key={item.opportunity.id} className="rounded-xl border border-slate-200 p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <div className="text-sm font-semibold text-slate-800">#{item.rank} · {item.title}</div>
                          ...
                          <span className={`flex-none text-xs px-2 py-1 rounded-full ${item.requiresApproval ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
                            {item.requiresApproval ? 'Requiere aprobación' : 'Auto (SAFE)'}
                          </span>
                        </div>
                        <div className="flex flex-wrap gap-2 mt-2 text-[11px] text-slate-500">
                          <span className="px-2 py-0.5 bg-slate-100 rounded-full">Impacto: {item.impact}</span>
                          <span className="px-2 py-0.5 bg-slate-100 rounded-full">Confianza: {item.confidence}</span>
                          <span className="px-2 py-0.5 bg-slate-100 rounded-full">Esfuerzo: {item.effort}</span>
                          <span className="px-2 py-0.5 bg-slate-100 rounded-full">Métrica: {item.metric}</span>
                          <span className="px-2 py-0.5 bg-slate-100 rounded-full">Medición: {item.opportunity.measurementWindowHours}h</span>
                        </div>
                        <details className="mt-2">
                          <summary className="text-xs text-blue-600 cursor-pointer">Ver evidencia</summary>
                          <ul className="mt-1 text-xs text-slate-600 space-y-0.5">
                            {item.opportunity.evidence.map((e, i) => (
                              <li key={i}>{e.source} · {e.metric}: <strong>{e.value}</strong>{e.note ? ` (${e.note})` : ''}</li>
                            ))}
                          </ul>
                        </details>
                      </li>
                  ...
                  <p className="text-xs text-slate-400">
                    {growth.summary} · Total oportunidades detectadas: {growth.opportunities.length}
                  </p>
                </div>
              ) : (
                <EmptyData label={growth?.summary || 'Sin oportunidades de crecimiento detectadas con los datos actuales.'} />
              )}
            </Card>
