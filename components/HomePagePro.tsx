        ...
        {items.map((item, i) => (
          <article key={item.id} className={`ni-hero__slide${i === idx ? ' is-active' : ''}`}>
            <div className="ni-hero__media">
              {item.imagen ? (
                <Image src={getResponsiveImageUrl(item.imagen, i === 0 ? 640 : 400)} alt={item.titulo} fill sizes="(max-width: 768px) 100vw, 50vw" style={{ objectFit: 'cover', objectPosition: 'center center' }} priority={i === 0} fetchPriority={i === 0 ? 'high' : 'auto'} crossOrigin="anonymous" unoptimized={false} />
              ) : null}
            </div>
          </article>
        ))}
