...
const EMISORAS = [
  ...
  { id: 'futura', name: 'Radio Futura', slogan: 'La emisora líder de Nicaragua', url: 'https://stream-204.zeno.fm/iwmolyow6tivv', genre: 'Urbano / Reggaetón / Electrónica', color: '#8b5cf6', freq: '91.3 FM', city: 'Managua' },
  ...
  { id: 'fiesta-latina', name: 'Fiesta Latina', slogan: '¡La fiesta no para!', url: 'https://radios.solumedia.com:6596/;', genre: 'Salsa / Merengue / Bachata / Reggaetón', color: '#ec4899', freq: '89.1 FM', city: 'Managua' },
  { id: 'clasica', name: 'Clásica 101.9', slogan: 'Tu tiempo, tu música', url: 'https://stream-281.zeno.fm/3dsfqnhmns8uv', genre: 'Oldies / Clásicos / Instrumental', color: '#06b6d4', freq: '101.9 FM', city: 'Managua' },
  { id: 'radio-ya', name: 'La Nueva Radio YA', slogan: 'La súper líder del dial', url: 'https://stream-281.zeno.fm/6nwu9a6uwz4tv', genre: 'Noticias / Deportes / Música', color: '#1e40af', freq: '99.1 FM', city: 'Managua' },
  { id: 'la-tuani', name: 'La Tuani', slogan: '¡La radio que te cautiva!', url: 'https://server.multimediamb.com/9300/;', genre: 'Pop / Hits / Contemporánea', color: '#d946ef', freq: '102.5 FM', city: 'Managua' },
];
...
export default function StickyRadio() {
  ...
  // Móvil minimizado → FAB 48x48
  if (isMobile && minimized) {
    return (
      <>
        <button
          onClick={() => setMinimized(false)}
          aria-label="Expandir reproductor de radio"
          style={{
            position: 'fixed',
            bottom: 80,
            right: 16,
            ...
            boxShadow: `0 4px 16px ${current.color}40, 0 2px 8px rgba(0,0,0,0.3)`,
            display: 'flex',
            alignItems: 'center',
        ...
        >
          {playing ? (
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 2, height: 18 }}>
              {[4, 10, 6, 12, 5].map((h, i) => (
                <span key={i} style={{ display: 'inline-block', width: 3, height: h, background: '#fff', borderRadius: 2, animation: `wvbar ${0.4 + (i % 3) * 0.15}s ease-in-out ${i * 0.05}s infinite alternate` }} />
              ))}
            </div>
          ) : (
  ...
  return (
    <>
      <div ref={containerRef} className="sticky-radio" style={{
        position: 'fixed', bottom: isMobile ? 56 : 0, left: 0, right: 0, zIndex: 200,
        ...
        <div className="sr-inner" style={{ maxWidth: 1400, margin: '0 auto', padding: '0 16px', display: 'flex', alignItems: 'center', gap: 12, width: '100%', height: isMobile ? 48 : 64, position: 'relative' }}>

          {/* Logo + indicador */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            <div style={{ width: isMobile ? 28 : 36, height: isMobile ? 28 : 36, borderRadius: '50%', background: playing ? `${current.color}20` : 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: `2px solid ${playing ? current.color : 'rgba(255,255,255,0.1)'}`, transition: 'all 0.3s ease' }}>
              <Radio size={isMobile ? 14 : 18} style={{ color: playing ? current.color : '#64748b', transition: 'color 0.3s' }} />
            </div>
            {!isMobile && (
          ...
          <button onClick={togglePlay} aria-label={playing ? 'Pausar radio' : 'Reproducir radio'} disabled={loading} className={playing ? 'play-btn-pulse' : ''}
            ...
            {loading ? <span style={{ display: 'inline-block', animation: 'spin 0.8s linear infinite', fontSize: isMobile ? 14 : 18 }}>⟳</span>
              : playing ? <Pause size={isMobile ? 14 : 16} /> : <Play size={isMobile ? 14 : 16} style={{ marginLeft: 2 }} />}
          </button>
          ...
          <div style={{ position: 'relative', flex: 1, minWidth: 0, maxWidth: 320 }}>
            <button onClick={() => setOpen(!open)} aria-label="Seleccionar emisora" aria-expanded={open}
              style={{ background: open ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.05)', border: `1px solid ${open ? current.color + '60' : 'rgba(255,255,255,0.08)'}`, color: '#e2e8f0', borderRadius: 10, padding: isMobile ? '6px 10px' : '8px 14px', cursor: 'pointer', width: '100%', textAlign: 'left', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, fontSize: isMobile ? 12 : 13, fontWeight: 600, transition: 'all 0.2s' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, overflow: 'hidden' }}>
                ...
                {!isMobile && <span style={{ fontSize: 10, color: '#64748b', fontWeight: 400, flexShrink: 0 }}>{current.freq}</span>}
              </div>
          ...
          {/* Minimizar (móvil) / Cerrar (desktop) */}
          {isMobile ? (
            <button onClick={() => setMinimized(true)} aria-label="Minimizar radio"
              style={{ flexShrink: 0, background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', padding: '6px', borderRadius: 6, transition: 'all 0.2s' }}>
              <ChevronUp size={18} />
            </button>
          ) : (
