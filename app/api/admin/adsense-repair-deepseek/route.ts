...
// DeepSeek da $5 gratis al registrarte. Precio: ~$0.14/M tokens input.
// Registro: https://platform.deepseek.com/api_keys
// =============================================================================
...
function verificarAuth(request: NextRequest): boolean {
  ...
  if (!validToken) {
    console.warn('[adsense-repair-deepseek] Ni ADMIN_API_KEY ni TOKEN_DE_LIMPIEZA_DE_ADMINISTRADOR configurados');
    return false;
  }
...
async function generarExpansionDeepSeek(titulo: string, contenido: string, resumen: string): Promise<string | null> {
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const doc of snapshot.docs) {
      ...
      if (!dryRun) {
        try {
          nuevoContenido = await generarExpansionDeepSeek(
            data.titulo || '',
            contenidoActual,
            data.resumen || ''
          );
    ...
    console.error('[adsense-repair-deepseek] Error:', error);
