...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (const docSnap of snap.docs) {
      ...
      const parrafo = expansiones[slug];
      if (!parrafo) continue;

      const contenido = data.contenido || '';
      if (contenido.includes(parrafo)) {
        skipped++;
        results.push(`SKIP: ${slug}`);
        continue;
      }
    ...
    return NextResponse.json({ success: true, updated, skipped, results });
  } catch (err: any) {
