...
const PATRONES: CategoryPattern[] = [
  ...
  { categoria: 'Turismo', keywords: /\b(?:mirador|mirador\s+de|lugar\s+tur[ií]stico|destino\s+tur[ií]stico|atractivo\s+tur[ií]stico|reserva\s+natural|parque\s+nacional|volc[aá]n|isla|playa|laguna|sendero|calet[oó]n|cañón|cascada|catarina|san\s+juan\s+del\s+sur|granada|le[oó]n|ometepe|mombacho|masaya|cerro\s+negro|laguna\s+de\s+apoyo|ruta\s+tur[ií]stica|gu[ií]a\s+tur[ií]stica|c[oó]mo\s+llegar|horarios\s+de\s+visita|precio\s+de\s+entrada)\b/i, peso: 3 },
